...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, onChangeView, language, setLanguage, userRole = 'client' }) => {
  ...
  const NavItem = ({ view, icon, labelKey }: { view: ViewState; icon: string; labelKey: string }) => {
    ...
    return (
      ...
      >
        ...
        <span className={`hidden md:block font-bold text-[10px] uppercase tracking-[0.2em] transition-all ${isActive ? 'text-white' : ''}`}>{t(labelKey)}</span>
      </button>
    );
  };

  const SectionLabel = ({ labelKey }: { labelKey: string }) => (
    <div className="px-8 py-4 mt-6 mb-2 hidden md:block">
  ...
  return (
    <aside className="w-20 md:w-72 h-full bg-brand-black/95 backdrop-blur-xl border-r border-white/5 flex flex-col shrink-0 z-[100] selection:bg-brand-platinum/30">
      {/* Brand */}
